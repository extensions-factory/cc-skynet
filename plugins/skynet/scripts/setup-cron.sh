...
CRON_CMD="0 0 * * * bash -lc 'LOG_DIR=\"\$HOME/.claude/logs\"; mkdir -p \"\$LOG_DIR\"; LOG_FILE=\"\$LOG_DIR/skynet-\$(date +\\%Y-\\%m-\\%d).log\"; f=\$(ls \$HOME/.claude/plugins/cache/cc-skynet/skynet/*/scripts/skills-fetch.sh 2>/dev/null | sort | tail -1); [ -n \"\$f\" ] && bash \"\$f\" >> \"\$LOG_FILE\" 2>&1' # $MARKER"
